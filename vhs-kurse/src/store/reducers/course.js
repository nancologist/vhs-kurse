...
const reducer = (state = initialState, action) => {
    ...
    if (action.type === 'BARRIER_FREE') {
        let updatedCourses = state.courses.filter(course => course.veranstaltungsort.barrierefrei === 'true');
        return {
            ...state,
            filteredCourses: updateCoursesMore(updatedCourses),
...
const updateCoursesMore = (state, courses, actionType) => {

    return updatedCourses;
};

const updateSearchFilters = (state) => {};

// const getAccessibleCourses = (state, courseArr) => {
//     const updatedCourses = state[courseArr].filter(course => course.veranstaltungsort.barrierefrei === 'true');
//     return {
//         ...state,
